...
public class APINameBuilder implements JPAEdmNameBuilder {
	...
	public String buildEntitySetName(final String entityTypeName) {
    	...
    	eSetName = "Departments".equals(entityTypeName) ? "Departments" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Students".equals(entityTypeName) ? "Students" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Lecturers".equals(entityTypeName) ? "Lecturers" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "College".equals(entityTypeName) ? "Colleges" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Courses".equals(entityTypeName) ? "Courses" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "PreviousEducation".equals(entityTypeName) ? "PreviousEducations" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Subjects".equals(entityTypeName) ? "Subjects" : defaultNameBuilder.buildEntitySetName(entityTypeName);
        return eSetName;
    }
